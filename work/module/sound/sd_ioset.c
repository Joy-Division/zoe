	...
	for (i = 0; i < 0x2C; i++){
		if (spu_tr_wk[i].vol_fg){
			if (i < 0x18){
				...
				sceSdSetParam( ((i-0x18)*2) | 1, spu_tr_wk[i].vol_l );
				sceSdSetParam( ((i-0x18)*2) | 0x101, spu_tr_wk[i].vol_r );
			}
			spu_tr_wk[i].vol_fg = 0;
		}
		if (spu_tr_wk[i].pitch_fg){
			if (i < 0x18){
				sceSdSetParam( (i * 2) | 0x200, spu_tr_wk[i].pitch );
			} else {
				sceSdSetParam( ((i-0x18)*2) | 0x201, spu_tr_wk[i].pitch );
			}
		...
		if (spu_tr_wk[i].addr_fg){
			if (i < 0x18){
				...
				sceSdSetAddr( ((i-0x18)*2) | 0x2041, (u_int)(spu_wave_start_ptr+spu_tr_wk[i].addr) );
			}
		...
		if (spu_tr_wk[i].env1_fg || spu_tr_wk[i].env2_fg || spu_tr_wk[i].env3_fg){
			if (i < 0x18){
				...
				sceSdSetParam( ((i-0x18)*2) | 0x0301, ((spu_tr_wk[i].ar & 0x7F) << 8) | spu_tr_wk[i].a_mode | ((spu_tr_wk[i].dr & 0x0F) << 4) | (spu_tr_wk[i].sl & 0x0F) );
				sceSdSetParam( ((i-0x18)*2) | 0x0401, ((spu_tr_wk[i].sr & 0x7F) << 6) | spu_tr_wk[i].s_mode | spu_tr_wk[i].r_mode | (spu_tr_wk[i].rr & 0x1F) );
			}
...
{
	spu_tr_wk[a0+0x20].rr = 0;
	spu_tr_wk[a0+0x20].env3_fg = 1;
	song_end[1] |= 1 << (a0+8);
	keyoffs[1] |= 1 << (a0+8);
	sound_w[a0+0x20].mpointer = 0;
}
...
void tone_set( u_int a0 )
{
	...
	spu_tr_wk[mtrack].dr = ((temp2[8] & 0xF) & 0xFF) - 0xF;
	...
	spu_tr_wk[mtrack].sl = ((temp2[11] & 0xF) & 0xFF);
...
void vol_set( u_int a0 )
{
	...
	if (sptr->panmod == 2 && mtrack < 0x20){
		...
		spu_tr_wk[mtrack].vol_l = (temp2 = ((se_pant[0x40-temp] * a0) / 127) * sng_master_vol[mtrack]) >> 16;
		spu_tr_wk[mtrack].vol_fg = 1;
	} else {
		if (mtrack < 0x20 || !(se_playing[mtrack-0x20].kind)){
			temp = sptr->pand >> 8;
			if (temp >= 0x29){
				temp = 0x28;
			}
			...
			if (mtrack < 0x20){
				...
				spu_tr_wk[mtrack].vol_l = (temp2 = ((pant[0x28-temp] * a0) / 127) * sng_master_vol[mtrack]) >> 16;
			} else {
				spu_tr_wk[mtrack].vol_r = (pant[temp] * a0) / 127;
				spu_tr_wk[mtrack].vol_l = (pant[0x28-temp] * a0) / 127;
			}
			spu_tr_wk[mtrack].vol_fg = 1;
		} else {
			a0 = (temp2 = (a0+1) * se_vol[mtrack-0x20]) >> 6;
			temp = se_pan[mtrack-0x20];
			if (sound_mono_fg){
				temp = 0x20;
			}
			a0++;
			spu_tr_wk[mtrack].vol_r = (temp2 = (se_pant[temp] * a0) / 127) >> 7;
			spu_tr_wk[mtrack].vol_l = (temp2 = (se_pant[0x40-temp] * a0) / 127) >> 7;
			spu_tr_wk[mtrack].vol_fg = 1;
		}
	}
}
